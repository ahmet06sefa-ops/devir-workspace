...
# -*- coding: utf-8 -*-
"""
v10.92 — 10.000-Madde Evrensel Görünüm ve Arayüz (UI/UX) Kişiselleştirme Atölyesi ve Yönetim Merkezi Üreticisi.
Kullanıcının talimatı, tercihleri (Hem Ayarlar Hem Ana Ekran, Saf Siyah OLED varsayılan tema, Tam Markdown Tablo Kataloğu):
 1. 10 Ana Görünüm Kategorisi (1.000'er madde)
 2. 100 Görünüm Alt Başlığı (100'er madde, [G01-A]..[G10-J])
 3. Toplam 10.000 Benzersiz Görünüm ve Arayüz Öğesi (#1..#10000)
 4. Hem 10000-GORUNUM-VE-ARAYUZ-KATALOGU.md bağımsız Markdown tablo kataloğunu
 5. Hem de EvrenselGorunumAtolye.kt saf Kotlin döngü motorunu üretir.
"""

import os

KATEGORILER = [
    (1, "1. Renk Paletleri, Zeminler & Gece/Gündüz Modları"),
    (2, "2. Tipografi, Yazı Boyutları & Aileleri"),
    (3, "3. Kartlar, Köşe Yarıçapları & Kenarlık Stilleri"),
    (4, "4. Gölgeler, Yükseklikler & Dokunmatik Ripple Efektleri"),
    (5, "5. Tablolar, Konu Başlıkları & İlerleme Grafikleri"),
    (6, "6. Sayaç Kadrani, Pomodoro Temaları & Medyatik Arayüz"),
    (7, "7. Masaüstü Widgetları & Kilit Ekranı Komplikasyonları"),
    (8, "8. Namaz Vakitleri, İbadet Kartları & Diyanet Arayüzü"),
    (9, "9. Ayarlar Ekranı, Alt Başlık Kartları & Menüler"),
    (10, "10. İkon Setleri, Animasyon Hızları & E-Mürekkep Düzeni")
]

ALT_BASLIKLAR_BY_KAT = {
    1: [
        ("[G01-A]", "Ana Renk (Primary) Paleti & Dinamik Ton Özelleştirici"),
        ("[G01-B]", "İkincil Renk (Secondary) & Vurgu (Accent) Ton Kontrolü"),
        ("[G01-C]", "Arka Plan (Surface) Renkleri & Şafak/Gece Tonlaması"),
        ("[G01-D]", "Saf Siyah OLED Modu (%100 Siyah Zemin Sıfır Pil)"),
        ("[G01-E]", "Material You Dinamik Duvar Kağıdı Renk Uyumlaması"),
        ("[G01-F]", "Koyu / Açık Renk Geçiş Saati ve Otomatik Zamanlayıcı"),
        ("[G01-G]", "Göz Yormayan Pastel Ton Dengeleyici & Sepya Filtresi"),
        ("[G01-H]", "Arayüz Renk Kontrast Oranı (WCAG AAA Standart Uyumu)"),
        ("[G01-I]", "Kart Zeminleri ile Ekran Zemini Ayrışma Renk Tonları"),
        ("[G01-J]", "Diyalog, Pencere ve Alt Menü Renk Şablon Seçicisi")
    ],
    2: [
        ("[G02-A]", "Başlık Yazı Tipi Ailesi (Poppins, Serif, Monospace, Condensed)"),
        ("[G02-B]", "Metin ve Madde Yazı Tipi Ailesi & Kalınlık Ölçeği (300/400/600)"),
        ("[G02-C]", "Evrensel Yazı Boyutu Ölçeği (11sp Mini - 15sp Normal - 32sp Devasa)"),
        ("[G02-D]", "Satır Aralığı (Line Height) & Paragraf Boşluk Oranları (24dp-32dp)"),
        ("[G02-E]", "Widget & Kilit Ekranı Özel Font Boyutları (16sp Başlık / 14sp Satır)"),
        ("[G02-F]", "Rakamlar ve Sayaç Kadrani İçin Monospace (Sabit Aralıklı) Font"),
        ("[G02-G]", "Arapça Ezani Metin ve İbadet Kartı Özel Kaligrafi Fontu"),
        ("[G02-H]", "Okuma Ekranlarında Harf Aralığı (Letter Spacing) Yoğunluk Arayüzü"),
        ("[G02-I]", "İtalik, Kalın ve Vurgulu Metin Rengi Ayrıştırma Stilleri"),
        ("[G02-J]", "E-Kitap ve Not Okuma Düzeni İçin Serif/Sans-Serif Geçiş Kalkanı")
    ],
    3: [
        ("[G03-A]", "Evrensel Kart Köşe Yarıçapı Ölçeği (ga_kose_kucuk=12dp .. dev=24dp)"),
        ("[G03-B]", "Kenarlık (Border / Stroke) Kalınlığı (0dp Çerçevesiz - 2dp Kalın)"),
        ("[G03-C]", "Kenarlık Rengi & Kontrast Çizgisi Yoğunluğu Modülü"),
        ("[G03-D]", "3D Cam Etkisi (Glassmorphism) & Kart Saydamlık Seviyesi"),
        ("[G03-E]", "Kart İç Boşlukları (Padding) & Dış Margin Aralıkları"),
        ("[G03-F]", "Etkileşimli Kartlarda Dokunma Halinde Büyüme (Scale) Efekti"),
        ("[G03-G]", "Daire, Yuvarlatılmış Kare ve Keskin Köşe Form Seçicisi"),
        ("[G03-H]", "Kart Zemin Rengi Yarı Saydamlık Derecesi (0-100 Şeffaflık)"),
        ("[G03-I]", "Alt Üst Katman Kart Gruplarında Görsel Ayrım Yüzeyi"),
        ("[G03-J]", "Öne Çıkan Kartlarda Özel Çerçeve ve Parlama Çizgisi")
    ],
    4: [
        ("[G04-A]", "Kart ve Buton Gölgelendirme Yükseklik Seviyesi (0dp Düz - 8dp Derin)"),
        ("[G04-B]", "Dokunmatik Dalgalanma (Ripple Effect) Renk ve Saydamlığı"),
        ("[G04-C]", "Buton Gölgeleri & Tıklama Baskı Animasyonu Derinliği"),
        ("[G04-D]", "Yüzen Eylem Butonu (FAB) & Alt Bar Yükseklik Derecesi"),
        ("[G04-E]", "Koyu Temada Gölge Aydınlatması & Kenar Parlaması"),
        ("[G04-F]", "Dokunma Tepki Süresi (Touch Feedback) Titreşim ve Görsel Arayüz"),
        ("[G04-G]", "Kaydırma Listelerınde Gölge Taşması ve Fading Edge Stili"),
        ("[G04-H]", "Açılır Menü ve Modal Diyalog Derinlik Gölgelendirmesi"),
        ("[G04-I]", "Düz (Flat) Tasarım Modu İçin Tüm Gölgeleri Sıfırlama Anahtarı"),
        ("[G04-J]", "Neo-Morfizm ve Yumuşak Işık Gölgelendirme Deneyimleri")
    ],
    5: [
        ("[G05-A]", "Tablo ve Bölüm Konu Başlıklarını Göster/Gizle (Sade / Detaylı Arayüz)"),
        ("[G05-B]", "7-Günlük ve 30-Günlük İlerleme Çubuk Grafiği Renk Kodları"),
        ("[G05-C]", "Aylık Takvim Isı Haritası (Heatmap) Doluluk Renk Skalası"),
        ("[G05-D]", "Pasta Grafiği Dilim Kalınlığı & Alt Konu Gösterim Biçimi"),
        ("[G05-E]", "Çalışma Serisi (Streak) Kartı & 3-Saniyelik Bildirim Bandı Görünümü"),
        ("[G05-F]", "Tablo Hücre Kenarlık Çizgileri ve Izgara Saydamlığı"),
        ("[G05-G]", "Haftalık Grafiklerde Ortalama Çizgisi Vurgu Rengi"),
        ("[G05-H]", "Soru Çözüm Hız Radarı Dairesel Eksen Görünümü"),
        ("[G05-I]", "Konularım Müfredat İlerleme Çubuğu Yükseklik Derecesi"),
        ("[G05-J]", "Leitner Kutu Kartlarında Zorluk Renk Kodlaması")
    ],
    6: [
        ("[G06-A]", "Sayaç Kadrani Dairesel Çizgi Kalınlığı & Çember Yarıçapı"),
        ("[G06-B]", "Alev Temaları, Görsel Eko ve Sayaç Merkez Yazı Tipi"),
        ("[G06-C]", "Sayaç Ön Tanımlı Preset Butonları (5-10-25 Dk) Tasarım ve İkonları"),
        ("[G06-D]", "Arka Plan Medya Kumandası (Play/Pause, İleri, Geri) Kart Görünümü"),
        ("[G06-E]", "Odak Sesleri Ekolayzır Barları & Müzik Panel Biçimi"),
        ("[G06-F]", "Sayaç Kalan Süre Animasyon Akıcılığı ve Saniye Atlayışı"),
        ("[G06-G]", "Pomodoro Seansı Bitişinde Ekran Parlama Efekti"),
        ("[G06-H]", "Sayaç Butonlarında Dairesel ve Kapsül Form Seçimi"),
        ("[G06-I]", "Akustik Çevresel Ses Maskeleme Dalga Formu Görselleştirici"),
        ("[G06-J]", "Odak Modu Çalışırken Arka Plan Görev Bandı Karartması")
    ],
    7: [
        ("[G07-A]", "Widget Arka Plan Zemin Rengi & Yarı Saydamlık Derecesi (0-100)"),
        ("[G07-B]", "Widget Başlık ve Madde Satır Yüksekliği (24px-26px Standart Korunarak)"),
        ("[G07-C]", "Madde İşareti Çember Çapı (18px x 18px) ve Renk Düzeni"),
        ("[G07-D]", "Kilit Ekranı Odak Çipi Duvar Kağıdı Kontrast Uyumu"),
        ("[G07-E]", "Takvim & Planlama Widget Sınav Vurgu Renkleri"),
        ("[G07-F]", "Namaz Widget Kalan Süre İlerleme Barı Renk Geçișleri"),
        ("[G07-G]", "Özet Kokpit Widget Görev ve İstatistik Ayrım Çizgileri"),
        ("[G07-H]", "Glassmorphism Widget Saydam Cam Yüzey Parlaklığı"),
        ("[G07-I]", "Masaüstü Takvim Aracında Bugün Hücresi Çerçeve Stili"),
        ("[G07-J]", "Widget İkon Boyutları ve Metin Margin Hizalaması")
    ],
    8: [
        ("[G08-A]", "Namaz Vakitleri Kalan Süre Çubuk Grafiği Renkleri"),
        ("[G08-B]", "İmsak, Güneş, Öğle, İkindi, Akşam, Yatsı Vakit İkon Stilleri"),
        ("[G08-C]", "Kerahat Vakti Kalkan Bandı Uyarı Rengi ve Kalınlığı"),
        ("[G08-D]", "Diyanet Çevrimdışı Şehir Seçici Kart Yüzeyi"),
        ("[G08-E]", "Cuma Günü Özel Hatırlatma Kartı Zemin Deseni"),
        ("[G08-F]", "Arapça Hat Sanatı İbadet Başlığı Vurgu Tonu"),
        ("[G08-G]", "Namaz Kartı Üzerinde Sıradaki Vakit Ayrışma Işığı"),
        ("[G08-H]", "15 Şehir Diyanet Tablosu Satır İçi Renk Kontrastı"),
        ("[G08-I]", "İbadet Hatırlatma Pop-up Modalı Zemin Teması"),
        ("[G08-J]", "Ezan Saati Bildirim Çipi İkon ve Metin Uyumu")
    ],
    9: [
        ("[G09-A]", "Ayarlar 8 Tematik Kategori Kart Başlık İkonları ve Renkleri"),
        ("[G09-B]", "Ayarlar Satır İçi Buton ve Switch/Toggle Biçimleri"),
        ("[G09-C]", "1.000, 10.000 & Görünüm Atölyesi Çip Filtre Yükseklik ve Kenarlıkları"),
        ("[G09-D]", "Alt Aksiyon Barları (Seçili Uygula / Sıfırla) Buton Konumlandırması"),
        ("[G09-E]", "Modal Diyalog (Alert/BottomSheet) Köşe Yuvarlatması ve Zemin Rengi"),
        ("[G09-F]", "Üst Eylem Çubuğu (Toolbar) Sabitlik ve Gölgelendirme Stili"),
        ("[G09-G]", "Ayarlar Alt Başlık Açıklama Metni (Subtitle) Saydamlık Seviyesi"),
        ("[G09-H]", "Arama Kutusu (EditText) Odaklanma Halinde Çerçeve Rengi"),
        ("[G09-I]", "Atölye Liste Satırlarında (ListView) Ayırıcı Çizgi Kalınlığı"),
        ("[G09-J]", "Hızlı Çip Seçim Kartlarında İkon-Metin Hizalama Oranları")
    ],
    10: [
        ("[G10-A]", "Uygulama Geneli İkon Stili (Dolgu / Çizgisel / Pastel Çift Renk)"),
        ("[G10-B]", "Ekran Geçiş Animasyon Hızı (Hızlı 150ms - Normal 300ms - Devre Dışı 0ms)"),
        ("[G10-C]", "E-Mürekkep Modu (Animasyonsuz, Gölgesiz, Saf Siyah-Beyaz E-Kitap Görünümü)"),
        ("[G10-D]", "Kilit Ekranı Motivasyon İnfografik Duvar Kağıdı Şablonları"),
        ("[G10-E]", "Wear OS Akıllı Saat ve Bileklik Arayüzü Mini-Komplikasyon Renkleri"),
        ("[G10-F]", "Açılış Ekranı (Splash Screen) Logo ve Zemin Kontrastı"),
        ("[G10-G]", "Liste Kaydırma ve Yenileme (Pull-to-Refresh) İkon Stili"),
        ("[G10-H]", "Sistemsel Uyarı Bildirimi (Snackbar/Toast) Köşe Yarıçapı"),
        ("[G10-I]", "Ana Ekran Atölye Butonları (18 Buton) Çip Düzeni"),
        ("[G10-J]", "Bütüncül Görünüm Sentezi (Tüm Arayüzü Tek Tuşla Tema Paketine Geçirme)")
    ]
}
...
def uret_md_katalog():
    md = []
    md.append("# 10.000-MADDE EVRENSEL GÖRÜNÜM & ARAYÜZ (UI/UX) KİŞİSELLEŞTİRME KATALOĞU (#1 - #10000)\n\n")
    md.append("**Günlük Asistan (`com.gunlukasistan.app`) — 10 Ana Görünüm Kategorisi · 100 Alt Başlık · Evrensel Görünüm Yönetim Merkezi**\n\n")
    md.append("Kullanıcının talimatı doğrultusunda, uygulamanın tüm görsel görünümünün ve arayüzünün (renkler, fontlar, köşe yarıçapları, gölgeler, cam etkisi, widget tipografisi, animasyon hızları vb.) **10.000 benzersiz görünüm öğesi** ile denetlendiği ve yönetildiği katalogdur.\n")
    md.append("Her madde, uygulamadaki **Ayarlar > 🎨 10.000-Madde Görünüm & Arayüz Kişiselleştirme Atölyesi (`EvrenselGorunumActivity`)** ve **Ana Ekrandaki 🎨 butonu** üzerinden incelenebilir, seçilebilir ve **'🎨 Değiştir & Uygula'** ile anında tüm uygulamaya senkronize edilebilir.\n\n---\n\n")

    no = 1
    for kat_no, kat_adi in KATEGORILER:
        md.append(f"## {kat_adi} (#{no} - #{no+999})\n\n")
        sublist = ALT_BASLIKLAR_BY_KAT[kat_no]
        for sub_kod, sub_baslik in sublist:
            md.append(f"### {sub_kod} {sub_baslik} (#{no} - #{no+99})\n\n")
            md.append("| #No | Alt Başlık & Kategori | Görünüm Öğesi | Özelleştirme Mantığı | Seçili Değer |\n")
            md.append("|:---:|:---|:---|:---|:---:|\n")
            for i in range(100):
                oge = GORUNUM_OGELERI[i % len(GORUNUM_OGELERI)]
                kural = OZELLESTIRME_KURALLARI[(i // len(GORUNUM_OGELERI)) % len(OZELLESTIRME_KURALLARI)]
                katman = TEKNIK_KATMANLAR[(i // len(GORUNUM_OGELERI)) % len(TEKNIK_KATMANLAR)]
                baslik = f"{sub_baslik} — {oge} {katman}"
                aciklama = f"{sub_baslik} ({sub_kod}) kapsamında, {oge.lower()} {kural}"
                md.append(f"| **#{no}** | `{sub_kod}` {kat_adi.split('.')[1].strip()} | **{baslik}** | {aciklama} | `[ ]` |\n")
                no += 1
            md.append("\n---\n\n")
    
    md.append("**Günlük Asistan Geliştirici Ekibi · Avrupa/İstanbul · v10.92**\n")
    return "".join(md)
...
if __name__ == "__main__":
    ...
    with open("/home/user/10000-GORUNUM-VE-ARAYUZ-KATALOGU.md", "w", encoding="utf-8") as f:
        f.write(md_metin)
    print("BASARILI: 10000-GORUNUM-VE-ARAYUZ-KATALOGU.md uretildi.")

    kt_metin = uret_kotlin_motoru()
    with open("/home/user/GunlukAsistan/app/src/main/java/com/gunlukasistan/app/EvrenselGorunumAtolye.kt", "w", encoding="utf-8") as f:
        f.write(kt_metin)
    print("BASARILI: EvrenselGorunumAtolye.kt (10.000 gorunum madde Kotlin motoru) uretildi.")
